...
var provider = () => {
    ...
    async function init(workItemIds:Array<number>, populateDropDownDefaults:boolean) {
        ...
        selectColumn.change((eventData) => {
            ...
            boardService.getBoardColumnSplitAsync(selectedColumn.val().toString())
                .then((isSplit) => {
                    if (isSplit) {
    ...
    async function getFormData(workItemIds:Array<number>) { 
        ...
        boardModel.boardColumn = $(".board-column-select").val().toString();
        boardModel.boardRow = $(".board-row-select").val().toString();
